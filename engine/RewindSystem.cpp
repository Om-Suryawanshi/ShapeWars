...
void RewindSystem::triggerRewind(bool keyPress)
{
    if (m_recordedCount > 10)
    {
        if (keyPress && !m_rewinding)
        {
            m_rewindClock.restart();
        }

        m_rewinding = keyPress;
    }
}
...
float RewindSystem::bufferState() const
{
    return static_cast<float>(m_recordedCount) / static_cast<float>(maxFrameCount);
}